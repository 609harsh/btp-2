...
const wrapper = ` px-4 py-6 2xl:px-2 2xl:py-2 border-0 border-b-2 `
const idleStyle = `border-color1-dark`
const errorStyle = ` border-error`
const inputStyle = `w-full text-3xl 2xl:text-lg 2sm:text-base outline-none font-light`

export function EmailInput({ value, setValue, error }) {
  return (
    <div className={`${wrapper} ${error ? errorStyle : idleStyle}`}>
      <input
...
export function PasswordInput({ value, setValue, error }) {
  const [visible, setVisible] = useState(false)
  return (
    <div className={`relative ${wrapper} ${error ? errorStyle : idleStyle}`}>
      <input
        className={`${inputStyle}`}
        value={value}
        onChange={(e) => setValue(e.target.value)}
